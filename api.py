...
def jobs():
    if request.method == 'GET':
        ...
        jobs_item = session.query(Jobs).all()
        return jsonify({
            "jobs": [
                job.to_dict(only=("id", "team_leader", "job", "work_size", "collaborators", "start_date", "end_date", "is_finished")) for job in jobs_item
            ]
        })
...
def one_jobs(job_id):
    if request.method == 'GET':
        session = db_session.create_session()
        job_item = session.query(Jobs).filter(Jobs.id == job_id).first()
        return jsonify(job_item.to_dict(only=("id", "team_leader", "job", "work_size", "collaborators", "start_date", "end_date", "is_finished")))
    return jsonify({"error": "Method Not Allowed"}), 405
